...
try:
    ...
    avg_transaction = total_sales / len(df_selection) if len(df_selection) > 0 else 0
    ...
    # CHART B: Sales by Hour (Peak Times)
    ...
    # CHART D: Revenue by City
    sales_by_city = df_selection.groupby(by=["City"]).sum()[["Sales"]]
    fig_city = px.bar(
    ...
    rating_by_product = df_selection.groupby(by=["Product line"]).mean()[["Rating"]]
    ...
    with st.expander("View Filtered Dataset"):
        st.dataframe(df_selection.style.format(precision=2))
